<#
.SYNOPSIS
    Exports a folder/file storage tree for every SharePoint Online site.
 
.DESCRIPTION
    Discovers all site collections, walks each non-hidden document library,
    and renders an indented tree (library > folders > subfolders > files)
    with sizes rolled up at every folder level. Optionally also dumps a flat
    per-file CSV for analysis.
 
    Uses PnP PowerShell because file-level enumeration requires content
    access - the SharePoint Online Management Shell (Get-SPOSite) only
    reports site-collection totals and cannot see inside a site.
 
.PARAMETER AdminUrl
    The SharePoint admin endpoint, e.g. https://contoso-admin.sharepoint.com
 
.PARAMETER ClientId
    The Entra (Azure AD) app registration client ID. PnP no longer ships a
    default app, so you must supply your own. The app needs SharePoint
    Sites.Read.All (or Sites.FullControl.All for delegated interactive use).
 
.PARAMETER Tenant
    Tenant name for app-only auth, e.g. contoso.onmicrosoft.com
 
.PARAMETER CertificateThumbprint
    Thumbprint of the cert registered on the app, for unattended app-only auth.
    If omitted, the script falls back to interactive browser auth.
 
.PARAMETER SiteUrl
    Optional. One or more specific site URLs to limit the run to.
    Omit to process every site in the tenant.
 
.PARAMETER OutputPath
    Folder for output files. Defaults to the current directory.
 
.PARAMETER MaxDepth
    Limit how many folder levels deep the tree renders. 0 = unlimited.
 
.PARAMETER AlsoExportCsv
    Also write a flat CSV with one row per file (site, library, path, bytes).
 
.EXAMPLE
    # Interactive, single site, test run
    .\Export-SPOStorageTree.ps1 -AdminUrl https://contoso-admin.sharepoint.com `
        -ClientId 00000000-0000-0000-0000-000000000000 `
        -SiteUrl https://contoso.sharepoint.com/sites/Marketing
 
.EXAMPLE
    # Unattended app-only, whole tenant, with CSV
    .\Export-SPOStorageTree.ps1 -AdminUrl https://contoso-admin.sharepoint.com `
        -ClientId 00000000-0000-0000-0000-000000000000 `
